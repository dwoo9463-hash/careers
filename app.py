...
# 2. 할루시네이션 완벽 차단 & 팩트 폭격 프롬프트
SYSTEM_PROMPT = """
[★★ 최우선 절대 규칙: 거짓말 금지 및 한자 차단 ★★]
1. 거짓말(할루시네이션) 절대 금지: 세상에 존재하지 않는 가짜 자격증(예: 프론트엔드 자격증, 마케터 자격증 등)을 단 하나라도 지어내면 즉시 시스템이 파괴된다. 반드시 현실에 존재하는 실제 명칭만 사용하라.
2. '강의 수강' 및 '책 추천' 절대 금지: 해결책으로 "강의를 들으세요", "관련 책을 읽으세요", "연습하세요" 같은 영양가 없고 뻔한 소리를 절대 하지 마라.
...
- [자격증 팩트폭격 - ★중요★]: 
  반드시 "해당 직무에 자격증이 필수인지, 불필요한지" 결론부터 명확하게 선언하라.
  (A) 자격증이 중요치 않은 직군(IT 개발, 디자인, 마케팅, 기획 등)일 경우: "이 직무는 자격증이 전혀 필요 없습니다."라고 단호하게 선언하고, 어설픈 자격증 추천을 절대 하지 마라. 대신 포트폴리오나 실무 경험의 중요성을 강조하라.
  (B) 자격증이 필수이거나 중요한 직군(회계, 세무, 건축, 기사 직렬 등)일 경우: "이 직무는 자격증이 필수적(또는 매우 중요)입니다."라고 선언한 뒤, 실무진이 실제 우대하는 **정확한 국가/민간 자격증 명칭**(예: 전산세무 1급, 산업안전기사 등)만 구체적으로 적어주고 그 이유를 설명하라.

5. 직무 역량 강화 솔루션 (즉시 실행 가능한 3단계)
- 앞서 말했듯 "책 읽기, 강의 듣기"는 절대 금지한다.
    ...
    with st.chat_message("assistant"):
        with st.spinner("가짜 정보를 걸러내고 팩트 기반의 상세 리포트를 작성 중입니다..."):
            chat_completion = client.chat.completions.create(
                ...
                temperature=0.0, # 창의성을 완전히 0으로 만들어서 헛소리를 원천 봉쇄함
                max_tokens=4000
            )
